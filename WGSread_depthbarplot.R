...
setwd('~/R_scripts')
reads3 <- reads <- read.csv("WGS_read_depths.csv")
...
readplot <- readplot + geom_bar(stat = "identity", width = .75, 
            ...
            theme_classic() +
            labs(title = 'WGS', x = "Sample ID", y = "Average Read Depth") +
            scale_x_discrete(labels = abbreviate, expand = c(0,0)) +
            scale_y_continuous(labels = c("0","10x","20x","30x","40x","50x"),
                               breaks = seq(0,50,10), 
                               expand = c(0,0), limit = c(0,50)) +
            theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 9))

ggsave('WGSrd_plot.png', readplot, width = 8, height = 3, dpi = 1200)